...
class SurfaceAreaAnalyzer:
    def __init__(self, results_path):
        self.results_path = results_path
        self.nph_values = []      # List of (id, value)
        self.non_nph_values = []  # List of (id, value)
        self.nph_data = []        # List of full data tuples (id, total_vol, surface_area)
    ...
    def load_data(self):
        ...
        for match in matches:
            ...
            vs_ratio = float(match[4])
            
            # 從 V/SA ratio 計算表面積: SA = Volume / Ratio
            surface_area = total_vol / vs_ratio if vs_ratio > 0 else 0
            
            data_tuple = (case_id, total_vol, surface_area)
            
            if '⚠️ NPH' in case_id:
                clean_id = case_id.replace(' ⚠️ NPH', '')
                self.nph_values.append((clean_id, surface_area))
                self.nph_data.append((clean_id, total_vol, surface_area))
            else:
    ...
    def get_statistics(self, data_list):
        """獲取統計數據"""
        sa_vals = [x[2] for x in data_list] if data_list else []
        vol_vals = [x[1] for x in data_list] if data_list else []
        ...
        return {
            ...
            'min': min(sa_vals),
            'max': max(sa_vals),
            'avg': sum(sa_vals)/count,
            'median': sorted(sa_vals)[count//2],
            'avg_vol': sum(vol_vals)/count
        }
    ...
    def generate_report(self, output_path):
        ...
        diff_sa = nph_stats['avg'] - non_stats['avg']
        diff_pct = (diff_sa / non_stats['avg']) * 100 if non_stats['avg'] else 0
        
        today = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
        
        report = f"""# 腦室表面積水腦症指標評估分析報告

**分析日期**: {today}
**數據來源**: 腦室表面積批次處理結果 ({self.n_nph + self.n_non} 個案例)

---

## 執行摘要

本報告評估「腦室表面積」作為水腦症 (NPH) 診斷指標的可行性。研究結果顯示,**此指標展現良好的鑑別能力**,NPH 組與非 NPH 組有 **{diff_pct:.1f}%** 的差異。
...
|---------|--------|--------|--------|--------|
| **表面積 (mm²)** | **{nph_stats['min']:.1f}** | **{nph_stats['max']:.1f}** | **{nph_stats['avg']:.1f}** | **{nph_stats['median']:.1f}** |

#### 非水腦症案例 (非 NPH, n={self.n_non})

| 測量指標 | 最小值 | 最大值 | 平均值 | 中位數 |
|---------|--------|--------|--------|--------|
| **表面積 (mm²)** | **{non_stats['min']:.1f}** | **{non_stats['max']:.1f}** | **{non_stats['avg']:.1f}** | **{non_stats['median']:.1f}** |

#### 組間差異

| 指標 | NPH 平均值 | 非 NPH 平均值 | 差異 | 差異百分比 |
|-----|-----------|-------------|------|-----------| 
| **表面積** | **{nph_stats['avg']:.1f} mm²** | **{non_stats['avg']:.1f} mm²** | **{diff_sa:+.1f} mm²** | **{diff_pct:+.1f}%** |
...
|------|--------|--------|-----|-----|--------|
"""
        for t in [15000, 18000, 20000, 22000, 25000]:
...
"""
    ...
    def generate_roc_curve(self, output_path):
        ...
        key_thresholds = [15000, 18000, 20000, 22000, 25000]
...
if __name__ == "__main__":
    analyzer = SurfaceAreaAnalyzer('/Users/lujingyuan/Project/3d-nph-indicators/result/volume_surface_ratio/results_summary.md')
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M")
    
    output_filename = f'result/volume_surface_ratio/surface_area_analysis_{timestamp}.md'
    analyzer.generate_report(os.path.join(os.getcwd(), output_filename))
    
    roc_filename = f'result/volume_surface_ratio/surface_area_roc_{timestamp}.png'
